...
class Repository:
    ...
    def login(self, username, password):
        with Session(self.engine) as db_session:
            current_user = db_session.query(User).filter(User.username==username and User.password==password).first()
            if current_user:
                return current_user
            else:
                return None
    ...
    def get_movies(self, limit=12):
        with Session(self.engine) as db_session:
            movies = db_session.query(Movie).limit(limit).all()
            return movies
    
    def get_movie_by_id(self, id):
        with Session(self.engine) as db_session:
            movie = db_session.query(Movie).filter(Movie.movieId == id).first()
            return movie
    
    def add_user(self, username, passsword):
        with Session(self.engine) as db_session:
            max_user_id = db_session.query(func.max(User.userId)).scalar()
            user = User(userId=max_user_id+1, username=username, password=passsword)
            result = db_session.add(user)
            db_session.commit()
            if result:
                return True
            else: return False
    ...
    def get_rating_by_user_movie(self, userId, movieId):
        with Session(self.engine) as db_session:
            rating = db_session.query(Rating).filter(Rating.movieId == movieId and Rating.userId == userId).first().rating
            if not rating:
                return None
            return rating
    ...
    def get_rating_object_by_user_movie(self, userId, movieId):
        with Session(self.engine) as db_session:
            rating = db_session.query(Rating).filter(Rating.movieId == movieId and Rating.userId == userId).first()
            if not rating:
                return None
            return rating
    ...
    def get_top_rating_movies(self, limit=12):
        with Session(self.engine) as db_session:
            ...
            top_rating_moiveID = db_session.query(Rating.movieId, func.avg(Rating.rating).label('avgRating')).group_by(Rating.movieId).having(func.count(Rating.rating) > 150).order_by(desc('avgRating')).limit(limit).all()
            movies = []
    ...
    def get_latest_movies(self, limit=12):
        with Session(self.engine) as db_session:
            lastest_movies = db_session.query(Movie).order_by(Movie.releaseDate.desc()).filter(Movie.releaseDate != "None" and Movie.releaseDate != "").limit(limit).all()
            return lastest_movies
    
    def get_average_rating_movie_by_id(self, id):
        with Session(self.engine) as db_session:
            avgRating_movie = db_session.query(func.avg(Rating.rating).label('avgRating')).group_by(Rating.movieId).filter(Rating.movieId==id).first()
    ...
    def get_cartoon_movie(self, limit=12):
        ...
        # return cartoons
        with Session(self.engine) as db_session:
            cartoons = db_session.query(Movie).filter(Movie.genres.contains('Animation')).limit(limit).all()
            return cartoons


    def get_star_rating_movie_by_id(self,id):
        with Session(self.engine) as db_session:
            ratings = db_session.query(Rating.rating).filter(Rating.movieId==id).all()
